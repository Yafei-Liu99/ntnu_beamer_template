...
% ============================================================
% Configure your phases here. Each entry: {Short label}{Date range}
% The number of entries must match the number of \sectionprogress calls.
\newcommand{\phaselist}{%
    {Background}{Sep--Dec '25}/
    {Literature}{Jan--Feb '26}/
    {Method}{Mar--May '26}/
    {Results}{Jun--Jul '26}/
    {Examples}{Aug--Sep '26}/
    {References}{Oct--Dec '26}%
}

\newcounter{sectionphase}

\newcommand{\sectionprogress}{%
  \stepcounter{sectionphase}%
  \begin{frame}{}
    \vfill
    \centering
    \begin{tikzpicture}[
      dot done/.style={
        circle, fill=ntnublue!45, minimum size=9pt, inner sep=0pt},
      dot active/.style={
        circle, fill=ntnublue, minimum size=12pt, inner sep=0pt,
        draw=ntnublue!60, line width=1.5pt},
      dot future/.style={
        circle, draw=ntnublue!25, fill=white, minimum size=9pt,
        inner sep=0pt, line width=0.8pt},
    ]
    \def\idx{0}%
    \def\mycurr{\thesectionphase}%
    %
    % Draw nodes
    \foreach \lbl/\dt in \phaselist {%
      \pgfmathtruncatemacro{\idx}{\idx+1}%
      \xdef\idx{\idx}%
      \pgfmathsetmacro{\xpos}{(\idx-1)*2.4}%
      %
      \ifnum\idx=\mycurr\relax
        % --- active ---
        \node[dot active] (N\idx) at (\xpos, 0) {};
        \node[above=10pt, font=\footnotesize\bfseries, text=ntnublue]
          at (N\idx) {\lbl};
        \node[below=10pt, font=\tiny\bfseries, text=ntnublue!80]
          at (N\idx) {\dt};
      \else\ifnum\idx<\mycurr\relax
        % --- done ---
        \node[dot done] (N\idx) at (\xpos, 0) {};
        \node[above=10pt, font=\scriptsize, text=gray]
          at (N\idx) {\lbl};
        \node[below=10pt, font=\tiny, text=gray!70]
          at (N\idx) {\dt};
      \else
        % --- future ---
        \node[dot future] (N\idx) at (\xpos, 0) {};
        \node[above=10pt, font=\scriptsize, text=gray!50]
          at (N\idx) {\lbl};
        \node[below=10pt, font=\tiny, text=gray!40]
          at (N\idx) {\dt};
      \fi\fi
    }%
    %
    ...
    \foreach \i in {1,...,\lastidx}{%
      ...
      \ifnum\nexti>\lastidx\relax\else
        \ifnum\nexti>\mycurr\relax
          \draw[ntnublue!15, line width=1.8pt] (N\i) -- (N\nexti);
        \else
          \draw[ntnublue!40, line width=1.8pt] (N\i) -- (N\nexti);
        \fi
      \fi
    }%
    %
    ...
    \foreach \i in {1,...,\lastidx}{%
      \ifnum\i<\mycurr\relax
        \node[font=\fontsize{5}{5}\selectfont, text=white] at (N\i) {\checkmark};
      \fi
    }%
    \end{tikzpicture}

    \vspace{1.8em}
    {\Large\color{ntnublue}\bfseries \insertsection}
    \vfill
  \end{frame}
}
